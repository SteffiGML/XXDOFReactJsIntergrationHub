...
const DesignationGrid = ({ source = APP_CONSTANT.FUSION }) => {
  const fusion = useGetFusionJobsQuery(undefined, {
    skip: source !== APP_CONSTANT.FUSION,
  });
  const darwin = useGetDarwinBoxDesignationsQuery(undefined, {
  ...
  if (isLoading)
    return <div className="p-4 text-gray-700">Loading {source} Jobs...</div>;
